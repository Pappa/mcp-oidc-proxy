...
- [ ] `uv run launch-mcp` starts the MCP server; README documents `uv run python -m nanoidp` for the auth server
- [ ] README covers full local dev workflow (both run commands, unit vs smoke tests, demo-only auth server warning)
